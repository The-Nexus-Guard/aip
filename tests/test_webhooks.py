...
def _make_agent():
    ...
    username = f"wh_{uuid.uuid4().hex[:8]}"
    return did, pub_b64, key, username
...
class TestWebhookCRUD:
    def test_create_webhook(self, local_service):
        did, pub, key, uname = _make_agent()
        requests.post(f"{local_service}/register", json={"did": did, "public_key": pub, "platform": "test", "username": uname})
...
class TestWebhookDeliveries:
    def test_delivery_log_empty(self, local_service):
        ...
        did, pub, key, uname = _make_agent()
        reg_resp = requests.post(f"{local_service}/register", json={"did": did, "public_key": pub, "platform": "test", "username": uname})
    ...
    def test_delivery_log_after_fire(self, local_service):
        ...
        did2, pub2, key2, uname2 = _make_agent()
        requests.post(f"{local_service}/register", json={"did": did2, "public_key": pub2, "platform": "test", "username": uname2})
